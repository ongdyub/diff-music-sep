...
from .ncsnpp_utils import layers
...
class HEncLayer(nn.Module):
    def __init__(self, chin, chout, kernel_size=8, stride=4, norm_groups=1, empty=False,
        ...
        self.temb_dense = nn.Linear(temb_dim, chout)
        self.temb_dense.weight.data = default_init()(self.temb_dense.weight.data.shape)
        nn.init.zeros_(self.temb_dense.bias)
        self.temb_act = nn.SiLU()

    def forward(self, x, inject=None, temb=None):
        """
        ...
        y = self.conv(x)
        assert y.dim() == 3 or y.dim() == 4
        if temb is not None:
            if y.dim() == 3:
                y += self.temb_dense(self.temb_act(temb))[:, :, None]
            elif y.dim() == 4:
                y += self.temb_dense(self.temb_act(temb))[:, :, None, None]
...
class HDecLayer(nn.Module):
    def __init__(self, chin, chout, last=False, kernel_size=8, stride=4, norm_groups=1, empty=False,
                 ...
                 context_freq=True, rewrite=True, temb_dim=None):
        """
    ...
    def forward(self, x, skip, length, temb=None):  

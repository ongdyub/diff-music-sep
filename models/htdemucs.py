...
class HTDemucs(nn.Module):
    ...
    def __init__(
        ...
        self.TEmbLayer = layerspp.GaussianFourierProjection(embedding_size=channels, scale=fourier_scale)
        ...
        for index in range(depth):
            ...
            enc = HEncLayer(
                chin_z, chout_z, dconv=dconv_mode & 1, context=context_enc, temb_dim=96, **kw
            )
            if freq:
                tenc = HEncLayer(
                    chin,
    ...
    def forward(self, mix, time_cond):
        temb = self.TEmbLayer(torch.log(time_cond)).to(mix.device)
        ...
        for idx, decode in enumerate(self.decoder):
            skip = saved.pop(-1)
            x, pre = decode(x, skip, lengths.pop(-1), temb=temb)
            ...
            if idx >= offset:
                ...
                if tdec.empty:
                    ...
                    pre = pre[:, :, 0]
                    xt, _ = tdec(pre, None, length_t, temb=temb)
                else:
                    skip = saved_t.pop(-1)
                    xt, _ = tdec(xt, skip, length_t, temb=temb)

...
class ScoreModelNCSNpp(torch.nn.Module):
    def __init__(
        ...
        backbone_args.update(
            num_channels_in=2, num_channels_out=2 * num_sources, 
            conditional=False, scale_by_sigma=False
        )
    ...
    def forward(self, x):
        ...
        # mask = torch.sigmoid(self.backbone(x, torch.ones(x.shape[0]).to(x.device)))
        # x = x.repeat(1,4,1,1)
        # x = x * mask
        x = self.backbone(x, torch.ones(x.shape[0]).to(x.device))
